...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, param: HyperParam):
        ...
        self.n_head = param.attention_head_number
        self.d_k = param.attention_key_dimension
        self.d_v = param.attention_value_dimension

        d_model = param.model_dimension
        d_k = param.attention_key_dimension
        d_v = param.attention_value_dimension
        n_head = param.attention_head_number
        ...
        self.attention = ScaledDotProductAttention(
            temperature=np.power(d_k, 0.5), attn_dropout=param.dropout)
